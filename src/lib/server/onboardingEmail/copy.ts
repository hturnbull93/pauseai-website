...
import type { BaseLanguage, ChapterBlockData, ChapterLink, IntentBucket } from './types.js'
...
const PROPOSAL_URL = 'https://pauseai.info/proposal'
const WELCOME_CALLS_URL = 'https://luma.com/PauseAI?tag=welcome'
...
function greeting(firstName: string): EmailBlock[] {
	return [{ type: 'heading', text: `Welcome to PauseAI, ${firstName}!` }]
}
...
function enNonVolunteer(
	...
	return {
		...
		signoff: [
			...
			{ type: 'paragraph', text: actNow ? 'Thanks again,' : 'Best,' },
			{ type: 'paragraph', text: GLOBAL_SIGNATURE }
		]
	}
}
...
function esVolunteer(firstName: string): EmailContent {
	return {
		...
		greeting: [{ type: 'heading', text: `¡Bienvenido a PauseAI, ${firstName}!` }],
		body: [
			{
				type: 'paragraph',
			...
			{ type: 'paragraph', text: `[Video Introductorio](${VIDEO_URL})` },
		...
		signoff: [
			...
			{ type: 'paragraph', text: 'Maxime y el equipo de PauseAI Global' },
			{ type: 'links', items: GLOBAL_SOCIALS }
		]
	}
}
